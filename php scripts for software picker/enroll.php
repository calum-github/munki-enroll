...
// this is the additionals manifest we are including
...
if ( file_exists( '../manifests/' . $client_identifier_path . $hostname ) )
    {
        echo "###-INFO-### Computer manifest already exists. Bailing out here.   ";
    }
else
    {
        echo "###-INFO-### Computer manifest does not exist. Creating directory structure as needed.   ";
...
// We also tell it to include the Room manifest
        ...
        // also add the additionals manifest - $additionals expands into 
        // $region/$school_name/$building/$level/$room/$room_manifest/Clients/$hostname_additionals
...
if ( file_exists( "../manifests/$region/$school_name/$building/$level/$room/$room_manifest" ) )
    {
        echo "###-INFO-### Room manifest already exists, no need to create it.   ";
    }
else
    {
        echo "###-INFO-### Room manifest NOT present, creating it now.   ";
...
if ( file_exists( "../manifests/$region/$school_name/$building/$level/$level_manifest" ) )
    {
        echo "###-INFO-### Level manifest already exists, no need to create it.   ";
    }
else
    {
        echo "###-INFO-### Level manifest NOT present, creating it now.   ";
...
if ( file_exists( "../manifests/$region/$school_name/$building/$building_manifest" ) )
    {
        echo "###-INFO-### Building manifest already exists, no need to create it.   ";
    }
else
    {
        echo "###-INFO-### Building manifest NOT present, creating it now.   ";
        // Create the new manifest for the Room
        $plist = new CFPropertyList();
        $plist->add( $dict = new CFDictionary() );
        // Add the manifest to production catalog
...
if ( file_exists( "../manifests/$region/$school_name/$school_manifest" ) )
    {
        echo "###-INFO-### School manifest already exists, no need to create it.   ";
    }
else
    {
        echo "###-INFO-### School manifest NOT present, creating it now.   ";
        // Create the new manifest for the Room
        $plist = new CFPropertyList();
        $plist->add( $dict = new CFDictionary() );
        // Add the manifest to production catalog
...
if ( file_exists( "../manifests/$region/$region_manifest" ) )
    {
        echo "###-INFO-### Region manifest already exists, no need to create it.   ";
    }
else
    {
        echo "###-INFO-### Region manifest NOT present, creating it now.   ";
        // Create the new manifest for the Room
        $plist = new CFPropertyList();
        $plist->add( $dict = new CFDictionary() );
        // Add the manifest to production catalog
